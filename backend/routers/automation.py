    ...
    try:
        with get_db_session() as db:
            ...
            if action == ACTION_FAMILY_CREATE:
                ...
                # 继承主号 Ultra 订阅状态
                if joined_group_id:
                    group = db.query(Group).filter(Group.id == joined_group_id).first()
                    if group and group.main_account_id:
                        main_acc = db.query(Account).filter(Account.id == group.main_account_id).first()
...
async def create_family_group(req: AccountActionRequest, db: Session = Depends(get_db)):
    ...
    if result.success:
        _save_cookies(req.account_id, profile_id)
    _sync_group_after_action(ACTION_FAMILY_CREATE, req.account_id, result.success, result.message)
    return result.to_dict()
...
async def accept_family_invite(req: AccountActionRequest, db: Session = Depends(get_db)):
    ...
    if result.success:
        _save_cookies(req.account_id, profile_id)
    _sync_group_after_action(ACTION_FAMILY_ACCEPT, req.account_id, result.success, result.message)
    return result.to_dict()
...
async def remove_family_member(req: FamilyMemberRequest, db: Session = Depends(get_db)):
    ...
    if result.success:
        _save_cookies(req.account_id, profile_id)
    _sync_group_after_action(ACTION_FAMILY_REMOVE, req.account_id, result.success, result.message, {"member_email": req.member_email})
    return result.to_dict()
...
async def leave_family_group(req: AccountActionRequest, db: Session = Depends(get_db)):
    ...
    if result.success:
        _save_cookies(req.account_id, profile_id)
    _sync_group_after_action(ACTION_FAMILY_LEAVE, req.account_id, result.success, result.message)
    return result.to_dict()
    ...
    try:
        while True:
            ...
            with get_db_session() as db:
                ...
                if action == ACTION_FAMILY_DISCOVER:
            ...
            if action == ACTION_LOGIN:
                ...
                for i, invite_email in enumerate(invite_emails):
                    ...
                    if invite_result and invite_result.success:
                        success_count += 1
                        _sync_group_after_action(ACTION_FAMILY_INVITE, account_id, True, invite_result.message, {"invite_email": invite_email})
                    else:
                ...
                for i, member_email in enumerate(member_emails):
                    ...
                    if remove_result and remove_result.success:
                        success_count += 1
                        _sync_group_after_action(ACTION_FAMILY_REMOVE, account_id, True, remove_result.message, {"member_email": member_email})
                    else:
                ...
                if not remove_result or not remove_result.success:
                    ...
                    _sync_group_after_action(ACTION_FAMILY_REMOVE, account_id, False, err_msg, {"member_email": old_email})
                    continue

                _sync_group_after_action(ACTION_FAMILY_REMOVE, account_id, True, remove_result.message, {"member_email": old_email})
                ...
                # 新成员在系统中, 尝试自动接受邀请
                step_offset[0] = 200
                on_step_replace({"type": "step", "name": PHASE_AUTO_ACCEPT, "status": "info", "message": f"{new_email} 在系统中, 将自动接受"})
                ...
                if not new_profile or not browser_manager.is_running(new_profile.id):
                    ...
                    try:
                        ...
                        with get_db_session() as db4:
                            ...
                            fresh_profile = db4.query(BrowserProfile).options(joinedload(BrowserProfile.account)).filter(BrowserProfile.id == new_profile.id).first()
                ...
                if need_auto_login:
                    step_offset[0] = 300
                    on_step_replace({"type": "step", "name": PHASE_AUTO_LOGIN, "status": "info", "message": new_email})
                    # 获取新成员的验证链接和辅助邮箱
                    new_verify_url = ""
                    new_recovery_email = ""
                    if new_account:
                ...
                if accept_result and accept_result.success:
                    _sync_group_after_action(ACTION_FAMILY_ACCEPT, new_account.id, True, accept_result.message, {"manager_account_id": account_id})
